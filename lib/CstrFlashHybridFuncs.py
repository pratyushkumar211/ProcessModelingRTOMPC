...
class KoopmanModel(tf.keras.Model):
    ...
    def __init__(self, Np, Ny, Nu, fN_dims):
        
        # Get a few sizes.
        Nz = Np*(Ny+Nu)
        Nxkp = Ny + Nz + fN_dims[-1]
        ...
        fN_layers = []
        for dim in fN_dims[1:-1]:
            fN_layers += [tf.keras.layers.Dense(dim, activation='tanh')]
        fN_layers += [tf.keras.layers.Dense(fN_dims[-1])]

        # Use the created layers to create the initial state.
        xkp0 = tf.concat((yz0, fnn(yz0, fN_layers)), axis=-1)
